...
export async function setPlan(plan, tenantId = DEMO_TENANT_ID, billing = {}) {
  ...
  // Column may be missing until migration is applied — retry without subscription id.
  if (error && patch.bachs_subscription_id !== undefined) {
    const { bachs_subscription_id: _drop, ...withoutSub } = patch;
    const retry = await db.from("tenants").update(withoutSub).eq("id", tenantId);
    error = retry.error;
  }
  if (error) throw error;
  return getTenant(tenantId);
